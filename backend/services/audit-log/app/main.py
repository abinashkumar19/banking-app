"""AuditLog microservice - generic CRUD against its own DynamoDB table."""
import uuid
...
app = FastAPI(title="VeeraBank AuditLog Service", version="1.0.0")
...
def get_item(item_id: str):
    ...
    item = resp.get("Item")
    if not item:
        raise HTTPException(status_code=404, detail="AuditLog item not found")
    return item
