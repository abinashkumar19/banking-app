"""Statements microservice - generic CRUD against its own DynamoDB table."""
import uuid
...
app = FastAPI(title="VeeraBank Statements Service", version="1.0.0")
...
@router.get("/")
def root():
    return {"service": "statements-service", "status": "running"}
...
def get_item(item_id: str):
    ...
    item = resp.get("Item")
    if not item:
        raise HTTPException(status_code=404, detail="Statements item not found")
    return item
