"""FraudDetection microservice - generic CRUD against its own DynamoDB table."""
import uuid
...
app = FastAPI(title="VeeraBank FraudDetection Service", version="1.0.0")
...
def get_item(item_id: str):
    ...
    item = resp.get("Item")
    if not item:
        raise HTTPException(status_code=404, detail="FraudDetection item not found")
    return item
